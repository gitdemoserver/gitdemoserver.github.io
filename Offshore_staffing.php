<!DOCTYPE html>
<html lang="zxx">
  <head>
    <meta charset="utf-8">
    <meta name="author" content="thecodude">
    <meta name="description" content="">
    <!-- responsive tag -->
    <meta http-equiv="x-ua-compatible" content="ie=edge">
    <meta name="viewport" content="width=device-width, initial-scale=1">
    <title>Offshore Staffing</title>
    <?php include('include.php') ?>
  </head>
  <body>
    <header class="site-header">
      <?php include('header.php') ?>
    </header>
    <section class="php-webbanner Offshore_staffing-banner">
      <div class="container">
        <div class="web-bannerinner">
            <div class="banner-leftcont">
              <h2>Offshore</h2>
              <h3>Staffing</h3>
              <p>Far beyond what’s expected from a PHP Development Company </p>
            </div>
            <div class="banner-rgtimg">
              <img src="images/Offshore_staffing-banner-right.png" class="img-fluid" alt="">
            </div>    
        </div>
      </div>
    </section>
    <section class="Solutions-provide">
      <div class="container">
        <div class="Solutions-provide-title" data-aos="zoom-in">
          <h2><span>Solutions</span> We Provide</h2>
          <h5>For Startups, Enterprises & Consumers</h5>
          <p>Do you dream of having a popular website like Facebook, Wikipedia, Yahoo!, Flickr, Tumblr? They are all built on PHP. AppVoxel, renowned as the top-level PHP web development company, offers a wide range of high-quality PHP development services.</p>
        </div>
        <div class="Solutions-provide-tiles">
          <ul data-aos="fade-up">
            <li>
              <div class="Solutions-topimg"><img src="images/solution-provide-01.png" alt=""></div>
              <h4>Custom Applications</h4>
            </li>
            <li>
              <div class="Solutions-topimg"><img src="images/solution-provide-02.png" alt=""></div>
              <h4>Dynamic Website</h4>
            </li>
            <li>
              <div class="Solutions-topimg"><img src="images/solution-provide-03.png" alt=""></div>
              <h4>CMS Applications</h4>
            </li>
            <li>
              <div class="Solutions-topimg"><img src="images/solution-provide-04.png" alt=""></div>
              <h4>PHP based E-commerce</h4>
            </li>
            <li>
              <div class="Solutions-topimg"><img src="images/solution-provide-05.png" alt=""></div>
              <h4>API Integration</h4>
            </li>
            <li>
              <div class="Solutions-topimg"><img src="images/solution-provide-06.png" alt=""></div>
              <h4>Cloud (Saas, PaaS)</h4>
            </li>
          </ul>
        </div>
      </div>
    </section>
    <section class="our-technology">
      <div class="container">
        <div class="our-technology-title" data-aos="fade-up">
          <h2>Our <span>Technology</span> Stack</h2>
          <p>We are a top-ranked PHP Development Company offering Premier PHP Web Application Services. AppVoxel PHP developers have a technical breadth and experience of developing fully customizable websites in PHP. We have helped startups, enterprises and business providers to launch their projects as per latest PHP application development standards and trends.</p>
        </div>
        <div class="our-technology-tab">
          <!-- Nav tabs -->
          <ul class="nav nav-tabs">
            <li class="nav-item">
              <a class="nav-link active" data-toggle="tab" href="#php-Versions">
                <span class="tab-icon php">
                  <?xml version="1.0" encoding="utf-8"?>
                  <!-- Generator: Adobe Illustrator 23.0.1, SVG Export Plug-In . SVG Version: 6.00 Build 0)  -->
                  <svg version="1.1" id="Layer_1" xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" x="0px" y="0px"
                    viewBox="0 0 75 38" style="enable-background:new 0 0 75 38;" xml:space="preserve">
                    <style type="text/css">
                      .st0{
                      fill-rule:evenodd;clip-rule:evenodd;fill:#3F8ADF;
                      }
                      .nav-link.active .st0{
                      fill: #0278ff
                      }
                    </style>
                    <path class="st0" d="M5.9,8h11.1c3.3,0,5.6,1,7.1,2.8c1.5,1.9,2,4.4,1.5,7.7c-0.2,1.5-0.6,3-1.3,4.3c-0.6,1.4-1.5,2.7-2.7,3.8
                      c-1.2,1.3-2.7,2.3-4.4,2.8c-1.6,0.4-3.2,0.6-4.9,0.6h-5L5.8,38H0L5.9,8 M10.8,12.8L8.3,25.3c0.2,0,0.3,0,0.5,0h0.6
                      c2.2,0.1,4.5-0.2,6.6-0.8c1.8-0.6,3-2.6,3.6-6.1c0.5-2.9,0-4.6-1.5-5.1c-1.8-0.5-3.7-0.7-5.5-0.6c-0.3,0-0.7,0-1,0L10.8,12.8
                      L10.8,12.8 M32.2,0h5.7l-1.6,8h5.1c2.8,0.1,4.9,0.6,6.3,1.8c1.4,1.1,1.8,3.2,1.2,6.4L46.2,30h-5.8L43,16.7c0.3-1,0.2-2.1-0.2-3
                      c-0.4-0.6-1.4-0.9-2.9-0.9l-4.6,0L31.9,30h-5.7L32.2,0 M55.1,8h11.1c3.3,0,5.6,1,7.1,2.8s2,4.4,1.5,7.7c-0.2,1.5-0.6,3-1.3,4.3
                      c-0.6,1.4-1.5,2.7-2.7,3.8c-1.2,1.3-2.7,2.3-4.4,2.8c-1.6,0.4-3.2,0.6-4.9,0.6h-5L55,38h-5.8L55.1,8 M60,12.8l-2.5,12.6
                      c0.2,0,0.3,0,0.5,0h0.6c2.2,0.1,4.5-0.2,6.6-0.8c1.8-0.6,3-2.6,3.6-6.1c0.5-2.9,0-4.6-1.5-5.1c-1.8-0.5-3.7-0.7-5.5-0.6
                      c-0.3,0-0.6,0-1,0L60,12.8L60,12.8"/>
                  </svg>
                </span>
                <span class="tab-txt">PHP Versions</span>
              </a>
            </li>
            <li class="nav-item">
              <a class="nav-link" data-toggle="tab" href="#Frameworks">
                <span class="tab-icon">
                  <?xml version="1.0" encoding="utf-8"?>
                  <!-- Generator: Adobe Illustrator 23.0.1, SVG Export Plug-In . SVG Version: 6.00 Build 0)  -->
                  <svg version="1.1" id="Layer_1" xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" x="0px" y="0px"
                    viewBox="0 0 70 70" style="enable-background:new 0 0 70 70;" xml:space="preserve">
                    <style type="text/css">
                      .st0{fill-rule:evenodd;clip-rule:evenodd;fill:#D7D7D7;}
                      .nav-link.active .st0{
                      fill: #0278ff
                      }
                    </style>
                    <path class="st0" d="M35.1,0c0.2,0.1,0.4,0.3,0.6,0.4C46.9,6,58,11.5,69.2,17.1c0.2,0.1,0.5,0.2,0.8,0.4c-0.3,0.2-0.5,0.3-0.7,0.4
                      C58,23.5,46.8,29.1,35.6,34.7c-0.4,0.2-0.9,0.2-1.3,0C23.1,29.1,12,23.5,0.8,17.9c-0.2-0.1-0.5-0.3-0.8-0.4c0.3-0.2,0.6-0.3,0.8-0.4
                      C11.9,11.5,23.1,5.9,34.2,0.4c0.2-0.1,0.4-0.3,0.6-0.4H35.1L35.1,0z M34.8,70c-1.6-0.8-3.2-1.7-4.8-2.5c-9.8-4.9-19.5-9.8-29.3-14.6
                      c-0.2-0.1-0.5-0.3-0.8-0.4c0.3-0.2,0.5-0.3,0.7-0.4c2.5-1.3,5-2.5,7.4-3.7c0.4-0.2,0.8-0.2,1.2,0c8.3,4.2,16.6,8.4,25,12.5
                      c0.4,0.3,1,0.3,1.4,0c8.3-4.2,16.7-8.4,25-12.5c0.3-0.2,0.7-0.2,1.1,0c2.5,1.3,5,2.5,7.5,3.8c0.2,0.1,0.4,0.2,0.7,0.4
                      c-0.3,0.2-0.5,0.3-0.8,0.4C58,58.5,46.9,64,35.7,69.6c-0.2,0.1-0.4,0.3-0.6,0.4H34.8z M70,35c-0.8,0.4-1.4,0.7-2,1
                      c-10.9,5.4-21.7,10.8-32.6,16.3c-0.3,0.1-0.6,0.2-0.9,0c-11.4-5.6-22.7-11.3-34-17C0.4,35.2,0.2,35.1,0,35c0.3-0.2,0.5-0.3,0.7-0.4
                      c2.5-1.2,5-2.5,7.4-3.7c0.4-0.2,0.8-0.2,1.1,0c8.3,4.2,16.7,8.4,25,12.5c0.4,0.2,0.9,0.2,1.3,0c8.4-4.2,16.7-8.4,25.1-12.5
                      c0.2-0.1,0.5-0.2,0.7-0.1C64.3,32.1,67.1,33.5,70,35z"/>
                  </svg>
                </span>
                <span class="tab-txt">Frameworks</span>
              </a>
            </li>
            <li class="nav-item">
              <a class="nav-link" data-toggle="tab" href="#Database">
                <span class="tab-icon">
                  <?xml version="1.0" encoding="utf-8"?>
                  <!-- Generator: Adobe Illustrator 23.0.1, SVG Export Plug-In . SVG Version: 6.00 Build 0)  -->
                  <svg version="1.1" id="Layer_1" xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" x="0px" y="0px"
                    viewBox="0 0 53 70" style="enable-background:new 0 0 53 70;" xml:space="preserve">
                    <style type="text/css">
                      .st0{fill-rule:evenodd;clip-rule:evenodd;fill:#D7D7D7;}
                      .nav-link.active .st0{
                      fill: #0278ff
                      }
                    </style>
                    <path class="st0" d="M23.3,70l-3.1-0.2c-4.5-0.2-8.9-1.1-13.2-2.4c-1.6-0.6-3.3-1.3-4.8-2.1c-0.5-0.3-0.9-0.6-1.3-0.9
                      C0.3,63.8,0,63,0,62.2c0-3.2,0-6.4,0-9.6v-0.8c1.3,0.6,2.5,1.2,3.8,1.7c3.7,1.4,7.6,2.4,11.5,2.9c4.6,0.6,9.3,0.9,13.9,0.7
                      C35.6,57,42,56,48,54c1.6-0.5,3-1.3,4.5-2c0.1-0.1,0.2-0.1,0.4-0.2c0,0.2,0,0.4,0,0.6c0,3.3,0,6.5,0,9.8c0,0.9-0.4,1.8-1.1,2.3
                      c-1.3,1-2.7,1.8-4.3,2.3c-3.6,1.3-7.3,2.2-11.2,2.6c-2.1,0.2-4.2,0.3-6.3,0.5c-0.2,0-0.3,0.1-0.5,0.1L23.3,70L23.3,70z M29.7,0
                      l3.1,0.2c4.9,0.3,9.8,1.2,14.5,2.9c1.4,0.5,2.7,1.1,3.9,1.9c2.5,1.6,2.4,3.1,0,4.8c-2,1.2-4.1,2.2-6.3,2.7c-4.5,1.3-9.2,2-14,2.1
                      c-3.6,0.1-7.2,0.1-10.8-0.1c-5.4-0.2-10.8-1.4-15.8-3.4c-0.9-0.4-1.9-1-2.7-1.5C1.1,9.3,0.8,9,0.5,8.6c-0.7-0.7-0.7-1.7,0-2.4
                      c0,0,0,0,0.1-0.1c0.5-0.6,1.2-1.1,1.8-1.5c2.3-1.3,4.7-2.2,7.2-2.8c4.2-1,8.5-1.6,12.8-1.8c0.4,0,0.7-0.1,1.1-0.1L29.7,0L29.7,0z
                      M53,14.9c0,0.3,0,0.4,0,0.6c0,3.3,0,6.6,0,9.8c0.1,0.8-0.3,1.5-0.9,2c-1,0.8-2.1,1.4-3.3,2c-3.5,1.5-7.2,2.5-11,3
                      c-5.1,0.8-10.3,1-15.5,0.7c-5.9-0.3-11.8-1.1-17.4-3.3c-1.3-0.5-2.5-1.2-3.6-1.9C0.4,27.2-0.1,26.1,0,25c0.1-3,0-6,0-8.9v-1.2
                      c0.1,0,0.2,0,0.3,0.1c3.4,1.9,7.2,3.2,11.1,3.9c7.2,1.4,14.5,1.8,21.8,1.2c5.5-0.3,10.9-1.4,16-3.3c1.2-0.5,2.3-1.1,3.5-1.7
                      C52.7,15,52.8,14.9,53,14.9L53,14.9z M53,33.3v6.1c0,1.4,0,2.8,0,4.2c0,0.9-0.3,1.7-1,2.3c-1.1,0.9-2.4,1.7-3.8,2.2
                      c-3.5,1.4-7.1,2.3-10.9,2.7c-5.1,0.7-10.3,0.9-15.4,0.6c-5.7-0.3-11.4-1.1-16.8-3.2c-1.3-0.5-2.5-1.2-3.7-1.9
                      c-1-0.6-1.6-1.7-1.5-2.8c0.1-3.2,0-6.3,0-9.5v-0.6c0,0,0.1,0,0.2,0c3.8,2.1,7.9,3.5,12.2,4.1c6.5,1.2,13.1,1.5,19.7,1.1
                      c5.7-0.2,11.3-1.3,16.6-3.2C50.1,34.7,51.5,34,53,33.3z"/>
                  </svg>
                </span>
                <span class="tab-txt">Database</span>
              </a>
            </li>
            <li class="nav-item">
              <a class="nav-link cms" data-toggle="tab" href="#CMS">
                <span class="tab-icon">
                  <?xml version="1.0" encoding="utf-8"?>
                  <!-- Generator: Adobe Illustrator 23.0.1, SVG Export Plug-In . SVG Version: 6.00 Build 0)  -->
                  <svg version="1.1" id="Layer_1" xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" x="0px" y="0px"
                    viewBox="0 0 69 70" style="enable-background:new 0 0 69 70;" xml:space="preserve">
                    <style type="text/css">
                      .st0{fill-rule:evenodd;clip-rule:evenodd;fill:#D7D7D7;}
                      .nav-link.active .st0{
                      fill: #0278ff
                      }
                    </style>
                    <path class="st0" d="M0,10.7C0.4,9.3,1.5,8,2.9,7.3c3.8-2.2,7.5-4.4,11.3-6.7c1.4-0.9,3.2-0.9,4.5,0c4,2.4,7.9,4.7,11.9,7
                      c1.5,0.8,2.3,2.3,2.3,4c0,4.8,0,9.6,0,14.4c0.1,1.5-0.7,2.9-2,3.6c-4.1,2.5-8.2,4.9-12.4,7.3c-1.3,0.8-2.9,0.8-4.2-0.1
                      c-3.9-2.3-7.7-4.6-11.6-6.8c-1.3-0.7-2.3-1.9-2.7-3.3L0,10.7L0,10.7z M16.9,26.5c1.5,0.1,2.9-0.4,4.1-1.3c1.1-0.8,1.8-2.1,1.9-3.5
                      c0.1-0.6-0.3-1.2-1-1.4c-0.6-0.2-1.2,0.1-1.4,0.7c-0.2,0.3-0.3,0.7-0.5,1.1c-0.6,1.3-1.9,2.1-3.3,2c-1.4,0.1-2.7-0.7-3.2-2
                      c-1-2.1-0.9-4.5,0.1-6.6c0.8-1.8,2.9-2.5,4.7-1.7c0.7,0.3,1.2,0.8,1.6,1.4c0.2,0.3,0.4,0.7,0.6,1c0.3,0.6,1,0.8,1.6,0.6
                      c0.6-0.3,0.9-0.9,0.8-1.6c-0.2-0.9-0.6-1.7-1.3-2.3c-2.8-2.6-7.1-2.4-9.7,0.4c-0.5,0.5-0.9,1.2-1.2,1.8c-0.6,1.4-0.8,2.9-0.7,4.4
                      C10.3,23.9,12.9,26.5,16.9,26.5z M69,26.9c-0.4,1.5-1.5,2.7-2.9,3.4c-3.7,2.1-7.4,4.3-11.1,6.6c-1.5,1-3.4,1-4.9,0
                      c-3.9-2.4-7.9-4.7-11.9-7c-1.4-0.7-2.2-2.2-2.1-3.8c0-4.8,0-9.6,0-14.4c-0.1-1.6,0.8-3.1,2.2-3.9c4-2.4,8-4.7,12-7.1
                      c1.3-0.9,3.1-0.9,4.4,0c3.8,2.2,7.5,4.5,11.3,6.7c1.4,0.7,2.5,1.9,2.9,3.4L69,26.9L69,26.9z M48.3,15c0,0,0.1,0,0.1,0
                      c0.3,1.2,0.6,2.5,1,3.7c0.5,2,1,4,1.5,5.9c0.1,0.7,0.6,1.3,1.4,1.5c0.9,0.2,1.7-0.3,2-1.1c0-0.1,0-0.1,0-0.2c0.6-2,1.1-4.1,1.6-6.1
                      c0.3-1.3,0.6-2.5,1-3.8c0.1,0.5,0.1,0.9,0.1,1.4c0,2.8,0,5.6,0,8.4c-0.1,0.7,0.4,1.4,1.2,1.5c0,0,0.1,0,0.2,0c0.7,0,1.3-0.5,1.4-1.3
                      c0-0.1,0-0.1,0-0.2c0-4,0-7.9,0-11.9c0.1-0.6-0.4-1.2-1-1.3c0,0-0.1,0-0.1,0c-0.6-0.1-1.2-0.1-1.8-0.1c-1.2,0-1.5,0.2-1.8,1.4
                      c-0.7,2.5-1.3,5-2,7.5c-0.1,0.2-0.1,0.5-0.2,0.7c-0.2-0.2-0.3-0.5-0.3-0.8c-0.7-2.5-1.3-5-2-7.5c0-0.8-0.7-1.4-1.5-1.4
                      c-0.1,0-0.2,0-0.3,0c-0.4,0-0.9,0-1.3,0c-1.3,0-1.7,0.5-1.7,1.8c0,3.7,0,7.3,0,10.9c0,0.3,0,0.5,0,0.8c0,0.7,0.7,1.3,1.4,1.3
                      c0.7,0,1.2-0.6,1.3-1.3c0-0.3,0-0.6,0-0.9C48.2,21,48.2,18,48.3,15L48.3,15z M51.2,51.2c0,2.4,0,4.8,0,7.3c0,1.5-0.8,2.9-2.1,3.7
                      c-4.2,2.4-8.3,4.9-12.5,7.3c-1.3,0.8-3,0.8-4.3,0c-4.1-2.4-8.3-4.8-12.4-7.2c-1.4-0.7-2.2-2.2-2.1-3.7c0-4.8,0-9.6,0-14.4
                      c-0.1-1.6,0.8-3,2.1-3.8c4.1-2.4,8.3-4.8,12.4-7.2c1.3-0.9,3.1-0.8,4.4,0c4.1,2.4,8.3,4.8,12.4,7.2c1.4,0.7,2.2,2.2,2.1,3.7
                      C51.2,46.3,51.2,48.8,51.2,51.2L51.2,51.2z M35,58.9c0.8-0.1,1.6-0.3,2.4-0.6c1.7-0.6,2.8-2.3,2.8-4.1c0.1-1.8-1-3.3-2.7-3.8
                      c-1.2-0.5-2.5-0.8-3.8-1.2c-0.5-0.2-1-0.4-1.4-0.7c-0.6-0.5-0.6-1.3-0.2-1.9c0,0,0-0.1,0.1-0.1c0.8-1,3.6-1.2,4.4,0.2
                      c0.2,0.5,0.5,0.9,0.8,1.3c0.3,0.5,1,0.7,1.6,0.3c0.1-0.1,0.2-0.2,0.3-0.3c0.4-0.5,0.5-1.2,0.2-1.7c-0.3-0.9-1-1.7-1.9-2.1
                      c-1.9-1-4.2-1-6.2-0.1c-1.4,0.6-2.2,1.9-2.2,3.4c-0.1,1.5,0.7,2.9,2.1,3.5c0.8,0.4,1.7,0.7,2.5,0.9c0.8,0.3,1.6,0.4,2.4,0.7
                      c0.7,0.2,1.2,0.9,1.2,1.6c0,0.7-0.3,1.4-1,1.8c-1.4,1-3.4,0.6-4.4-0.8c-0.1-0.2-0.2-0.4-0.3-0.5c-0.1-0.2-0.2-0.4-0.3-0.7
                      c-0.2-0.7-0.9-1.1-1.5-0.9c0,0-0.1,0-0.1,0c-0.7,0.3-1.1,1-0.8,1.7c0,0,0,0,0,0c0.2,0.7,0.5,1.4,1,2C31,58.5,32.7,58.8,35,58.9
                      L35,58.9z"/>
                  </svg>
                </span>
                <span class="tab-txt">CMS</span><span class="tab-txt">
              </a>
            </li>
            <li class="nav-item">
              <a class="nav-link" data-toggle="tab" href="#Frontend">
                <span class="tab-icon">
                  <?xml version="1.0" encoding="utf-8"?>
                  <!-- Generator: Adobe Illustrator 23.0.1, SVG Export Plug-In . SVG Version: 6.00 Build 0)  -->
                  <svg version="1.1" id="Layer_1" xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" x="0px" y="0px"
                    viewBox="0 0 70 70" style="enable-background:new 0 0 70 70;" xml:space="preserve">
                    <style type="text/css">
                      .st0{fill-rule:evenodd;clip-rule:evenodd;fill:#D7D7D7;}
                      .nav-link.active .st0{
                      fill: #0278ff
                      }
                    </style>
                    <path class="st0" d="M35,0h32.5C69.3,0,70,0.7,70,2.5v65c0,1.9-0.6,2.6-2.6,2.6h-65C0.7,70,0,69.3,0,67.5c0-21.7,0-43.3,0-65
                      C0,0.6,0.6,0,2.6,0H35z M66.1,11.7c-0.3,0-0.5-0.1-0.7-0.1c-9,0-18,0-27.1,0c-0.4,0-0.8,0.2-1.1,0.5c-2.1,2.1-4.2,4.1-6.2,6.3
                      c-0.6,0.7-1.6,1.1-2.5,1.1c-7.8,0-15.7,0-23.5,0H3.9V66h9.7c0-1.9,0-3.7,0-5.4c-0.2-1.1,0.5-2.1,1.6-2.2c0.1,0,0.3,0,0.4,0
                      c1.1-0.1,1.9-0.9,1.8-2c0-0.6,0-1.2,0-1.8c-0.2-0.1-0.4-0.1-0.5-0.2c-1-0.3-1.7-1.3-1.4-2.4c0.2-0.9,1-1.5,1.9-1.4
                      c1.2,0,2.5,0,3.7,0c0-0.1,0.1-0.3,0.1-0.4c0-4.3,0-8.5,0-12.8c-0.1-0.3-0.3-0.6-0.6-0.8c-2-0.8-3.3-2.8-3.2-5c0-2,0-4.1,0-6.1
                      c-0.1-1.1,0.7-2,1.7-2.1c0.1,0,0.2,0,0.4,0c2.5,0,5,0,7.5,0c1.1-0.1,2,0.7,2.1,1.7c0,0.1,0,0.2,0,0.4c0,2.1,0.1,4.2,0,6.3
                      c0,2.1-1.2,3.9-3.1,4.7c-0.5,0.1-0.8,0.6-0.7,1.1c0,4,0,8,0,12v0.8c0.2,0.1,0.2,0.1,0.2,0.1c1.1,0,2.2,0,3.4,0
                      c1.1-0.1,2.1,0.7,2.2,1.7c0.1,1-0.6,1.9-1.6,2.1c-0.1,0-0.2,0.1-0.3,0.2c0,0.6,0,1.2,0,1.8c0,0.9,0.7,1.7,1.6,1.9
                      c1.9,0.4,2.2,0.7,2.2,2.6v5.1h5.8V65c0-6.3,0-12.6,0-18.9c0.1-0.6-0.2-1.2-0.8-1.5c-5.3-3.7-6.6-10.9-2.9-16.2
                      c1.1-1.6,2.6-2.9,4.4-3.8c0.4-0.2,0.8-0.3,1.3-0.5v1.1c0,3,0,6,0,9c-0.1,0.5,0.2,1,0.7,1.2c1.1,0.5,2.1,1.4,3.2,1.4
                      c1,0,2.1-0.9,3.1-1.4c0.5-0.2,0.9-0.7,0.8-1.2c0-3,0-6,0-9c0-0.3,0-0.6,0-1.1c0.5,0.2,0.9,0.3,1.2,0.5c5.8,2.9,8.1,9.9,5.3,15.6
                      c-0.9,1.8-2.2,3.3-3.9,4.5c-0.5,0.3-0.8,0.8-0.7,1.4c0,6.3,0,12.7,0,19v1h15.5L66.1,11.7L66.1,11.7z M9.7,7.8c-1,0-1.9,0.8-1.9,1.9
                      c0,0,0,0,0,0c0,1.1,0.8,1.9,1.9,2c0,0,0,0,0.1,0c1.1,0,1.9-0.9,1.9-1.9C11.7,8.7,10.8,7.8,9.7,7.8C9.7,7.8,9.7,7.8,9.7,7.8z
                      M25.3,7.8c-1.1,0-1.9,0.8-2,1.9c0,0,0,0,0,0.1c0,1,0.9,1.9,1.9,1.9c1.1,0,1.9-0.8,2-1.9c0,0,0,0,0-0.1C27.2,8.7,26.4,7.8,25.3,7.8
                      C25.4,7.8,25.3,7.8,25.3,7.8L25.3,7.8z M17.5,7.8c-1.1,0-1.9,0.9-1.9,1.9c0,1.1,0.9,1.9,1.9,1.9s1.9-0.9,1.9-1.9c0,0,0,0,0,0
                      C19.4,8.7,18.6,7.8,17.5,7.8L17.5,7.8z"/>
                  </svg>
                </span>
                <span class="tab-txt">Frontend</span>
              </a>
            </li>
            <li class="nav-item">
              <a class="nav-link" data-toggle="tab" href="#web-services">
                <span class="tab-icon">
                  <?xml version="1.0" encoding="utf-8"?>
                  <!-- Generator: Adobe Illustrator 23.0.1, SVG Export Plug-In . SVG Version: 6.00 Build 0)  -->
                  <svg version="1.1" id="Layer_1" xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" x="0px" y="0px"
                    viewBox="0 0 69 70" style="enable-background:new 0 0 69 70;" xml:space="preserve">
                    <style type="text/css">
                      .st0{fill-rule:evenodd;clip-rule:evenodd;fill:#D7D7D7;}
                      .nav-link.active .st0{
                      fill: #0278ff
                      }
                    </style>
                    <path class="st0" d="M23.1,0c1.5,0.2,3.1,0.3,4.6,0.6c3.1,0.4,6,1.3,8.8,2.8C37.6,4.2,38.7,5,39.7,6c0.3,0.3,0.5,0.6,0.7,0.9
                      c1.4,2.8,0.9,4-1.2,5.4c-2.4,1.5-5,2.6-7.8,3c-8.1,1.7-16.4,1.5-24.3-0.8c-1.8-0.5-3.5-1.3-5.1-2.4c-2-1.4-2.4-2.6-1.2-5.1
                      c0.8-1.5,2-2.7,3.5-3.5c2.9-1.6,6-2.7,9.3-3.1C15,0.3,16.5,0.2,18,0H23.1L23.1,0z M19.7,70c-0.2-0.1-0.4-0.2-0.6-0.2
                      c-1.8-0.5-3.3-1.8-3.9-3.6c-0.1-0.4-0.4-0.6-0.8-0.6c-3.5,0-7,0-10.4,0H3v-2.9h0.8c3.4,0,6.9,0,10.3,0c0.4,0.1,0.8-0.2,0.9-0.6
                      c0.6-1.6,1.9-2.9,3.5-3.4c0.3-0.1,0.6-0.4,0.5-0.7c0-1.3,0-2.6,0-4C18,53.9,17,53.8,16,53.7c-3.8-0.3-7.6-1.3-11-3
                      c-1.5-0.7-2.7-1.8-3.8-3c-0.7-0.8-1.1-1.8-1.2-2.8c-0.1-2.4,0-4.7,0-7.1c0,0,0.1,0,0.1,0c3.7,2.2,7.9,3.6,12.2,4
                      c2.2,0.3,4.4,0.4,6.6,0.6c0.3,0,0.7,0.2,0.8,0.5c1.3,3.4,4.3,5.9,8,6.5c0.6,0.1,1.2,0.4,1.6,0.7c1.1,0.8,2.3,1.4,3.4,2.1
                      c-3.5,1.1-7.1,1.7-10.7,1.8v1.8c0,0.8,0,1.6,0,2.3c0,0.2,0.1,0.4,0.3,0.5c2,0.6,3.5,2.2,4,4.2h11.9v2.8H26.3
                      c-0.4,1.6-1.5,2.9-2.9,3.7c-0.6,0.3-1.3,0.5-1.9,0.7L19.7,70L19.7,70z M41.1,24.6c1.5-5.9,7.5-9.4,13.4-7.9c0.2,0.1,0.5,0.1,0.7,0.2
                      c5.6,1.8,8.7,7.8,6.9,13.4c-0.1,0.3-0.2,0.6-0.3,0.9c0.3,0.1,0.6,0.1,0.9,0.2c4.2,0.9,7,5,6.1,9.2c-0.2,0.8-0.4,1.5-0.8,2.2
                      c-2.3,4.4-6.9,6.3-12,4.8c-0.8-0.2-1.6-0.9-2.4-0.8c-0.7,0.1-1.3,0.9-2,1.3c-2.7,1.7-7.5,1.9-10.3-1.5c-0.4,0.3-0.8,0.7-1.2,1
                      c-2,1.7-4.8,2.2-7.2,1.2c-1-0.3-1.9-1-2.4-1.9c-0.2-0.2-0.5-0.3-0.8-0.4c-1.9,0-3.7-0.7-5.1-1.8c-4.1-3.4-3.2-10.2,1.7-12.7
                      c0.3-0.2,0.6-0.3,0.9-0.4c-0.8-4,0.6-7.1,4.3-9C34.8,21,38.7,21.8,41.1,24.6z M0,14.8c1.5,0.7,2.9,1.5,4.4,2
                      c3.5,1.3,7.2,2.1,10.9,2.3c4.6,0.5,9.3,0.3,13.8-0.4c4.2-0.7,8.2-1.7,11.8-4c0.1,0,0.1,0,0.2,0c0.1,1.1,0.1,2.3-0.1,3.4
                      c-0.2,0.7-0.6,1.3-1.2,1.8c-0.2,0-0.4,0-0.6-0.1c-5.4-2.3-11.6,0.1-14.1,5.4c-0.4,0.8-0.6,1.7-1,2.7c-1.4,0-2.8,0.1-4.3,0.1
                      c-4.5,0-9.1-0.7-13.4-2c-1.8-0.5-3.4-1.4-4.9-2.5c-1-0.7-1.6-1.9-1.5-3.1C0.1,18.5,0,16.6,0,14.8z M0,26.1c0.7,0.4,1.3,0.8,2,1.1
                      c3.3,1.6,6.9,2.7,10.6,3c3.2,0.3,6.4,0.4,9.7,0.6c0.2,0,0.3,0,0.6,0c-2.5,2.1-3.9,5.2-3.8,8.5c-0.1,0-0.1,0.1-0.2,0.1
                      c-5-0.2-10-0.8-14.7-2.9C3.1,36,2,35.3,1,34.5c-0.7-0.5-1-1.4-1-2.2C0.1,30.3,0,28.3,0,26.1z"/>
                  </svg>
                </span>
                <span class="tab-txt">Web Services</span>
              </a>
            </li>
            <li class="nav-item">
              <a class="nav-link" data-toggle="tab" href="#js-libraries">
                <span class="tab-icon">
                  <?xml version="1.0" encoding="utf-8"?>
                  <!-- Generator: Adobe Illustrator 23.0.1, SVG Export Plug-In . SVG Version: 6.00 Build 0)  -->
                  <svg version="1.1" id="Layer_1" xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" x="0px" y="0px"
                    viewBox="0 0 70 70" style="enable-background:new 0 0 70 70;" xml:space="preserve">
                    <style type="text/css">
                      .st0{fill-rule:evenodd;clip-rule:evenodd;fill:#D7D7D7;}
                      .nav-link.active .st0{
                      fill: #0278ff
                      }
                    </style>
                    <path class="st0" d="M70,0c-0.2,2-0.3,4-0.5,6c-0.5,5.3-1,10.7-1.5,16c-0.6,6.1-1.1,12.2-1.7,18.3c-0.5,5.2-1,10.5-1.4,15.7
                      c-0.2,2.6-0.5,5.2-0.7,7.8c-0.1,0.8-0.6,0.7-1,0.8c-4.6,0.9-9.1,1.8-13.7,2.8c-4.6,0.9-9.1,1.9-13.7,2.7c-1.2,0.1-2.4,0-3.5-0.3
                      c-8.5-1.7-17-3.4-25.5-5.1c-0.6,0-1.1-0.5-1.1-1.1c0,0,0-0.1,0-0.1C4.9,54.2,4,45.1,3.2,35.9c-0.7-7.2-1.3-14.4-1.9-21.7
                      C0.8,9.7,0.4,5.1,0,0.5C0,0.4,0,0.2,0,0H70L70,0z M49,38c-0.3,3.8-0.7,7.5-1,11.2c-0.1,0.3-0.3,0.5-0.6,0.6
                      c-1.2,0.3-2.3,0.5-3.5,0.7c-2,0.4-4,0.8-5.9,1.2v5.9c0.4-0.1,0.7-0.1,1.1-0.2c4.4-0.9,8.8-1.8,13.2-2.6c0.7,0,1.3-0.6,1.2-1.3
                      c0,0,0-0.1,0-0.1c0.4-4.8,0.9-9.6,1.3-14.4c0.2-2.2,0.4-4.5,0.6-6.8H43.8V17.5h12.9c0.2-2,0.4-3.9,0.5-5.8H37.9V38L49,38L49,38z
                      M32,57.6V11.7h-5.8v38.8c-1.2-0.2-2.3-0.5-3.4-0.7c-0.4,0-0.8-0.4-0.8-0.8c0,0,0-0.1,0-0.1c-0.1-1.7-0.3-3.3-0.4-5h-5.8
                      c0,0.1-0.1,0.1-0.1,0.2c0.3,3.3,0.6,6.6,0.9,9.9c0,0.4,0.3,0.7,0.7,0.7c0,0,0,0,0,0c1.7,0.3,3.5,0.7,5.2,1
                      C25.7,56.4,28.8,57,32,57.6z"/>
                  </svg>
                </span>
                <span class="tab-txt">JS Libraries</span>
              </a>
            </li>
          </ul>
          <!-- Tab panes -->
          <div class="tab-content">
            <div id="php-Versions" class="tab-pane active">
              <div class="tab-content-center-inner">
                <div class="tab-content-center">
                  <img src="images/php4x.png" alt="">
                  <p>PHP4.X</p>
                </div>
                <div class="tab-content-center">
                  <img src="images/php5x.png" alt="">
                  <p>PHP4.X</p>
                </div>
                <div class="tab-content-center">
                  <img src="images/php7x.png" alt="">
                  <p>PHP4.X</p>
                </div>
              </div>
            </div>
            <div id="Frameworks" class="tab-pane fade">
              <div class="tab-content-center-inner">
                <div class="tab-content-center">
                  <img src="images/php4x.png" alt="">
                  <p>PHP4.X</p>
                </div>
                <div class="tab-content-center">
                  <img src="images/php5x.png" alt="">
                  <p>PHP4.X</p>
                </div>
                <div class="tab-content-center">
                  <img src="images/php7x.png" alt="">
                  <p>PHP4.X</p>
                </div>
              </div>
            </div>
            <div id="Database" class="tab-pane fade">
              <div class="tab-content-center-inner">
                <div class="tab-content-center">
                  <img src="images/php4x.png" alt="">
                  <p>PHP4.X</p>
                </div>
                <div class="tab-content-center">
                  <img src="images/php5x.png" alt="">
                  <p>PHP4.X</p>
                </div>
                <div class="tab-content-center">
                  <img src="images/php7x.png" alt="">
                  <p>PHP4.X</p>
                </div>
              </div>
            </div>
            <div id="CMS" class="tab-pane fade">
              <div class="tab-content-center-inner">
                <div class="tab-content-center">
                  <img src="images/php4x.png" alt="">
                  <p>PHP4.X</p>
                </div>
                <div class="tab-content-center">
                  <img src="images/php5x.png" alt="">
                  <p>PHP4.X</p>
                </div>
                <div class="tab-content-center">
                  <img src="images/php7x.png" alt="">
                  <p>PHP4.X</p>
                </div>
              </div>
            </div>
            <div id="Frontend" class="tab-pane fade">
              <div class="tab-content-center-inner">
                <div class="tab-content-center">
                  <img src="images/php4x.png" alt="">
                  <p>PHP4.X</p>
                </div>
                <div class="tab-content-center">
                  <img src="images/php5x.png" alt="">
                  <p>PHP4.X</p>
                </div>
                <div class="tab-content-center">
                  <img src="images/php7x.png" alt="">
                  <p>PHP4.X</p>
                </div>
              </div>
            </div>
            <div id="web-services" class="tab-pane fade">
              <div class="tab-content-center-inner">
                <div class="tab-content-center">
                  <img src="images/php4x.png" alt="">
                  <p>PHP4.X</p>
                </div>
                <div class="tab-content-center">
                  <img src="images/php5x.png" alt="">
                  <p>PHP4.X</p>
                </div>
                <div class="tab-content-center">
                  <img src="images/php7x.png" alt="">
                  <p>PHP4.X</p>
                </div>
              </div>
            </div>
            <div id="js-libraries" class="tab-pane fade">
             <div class="tab-content-center-inner">
                <div class="tab-content-center">
                  <img src="images/php4x.png" alt="">
                  <p>PHP4.X</p>
                </div>
                <div class="tab-content-center">
                  <img src="images/php5x.png" alt="">
                  <p>PHP4.X</p>
                </div>
                <div class="tab-content-center">
                  <img src="images/php7x.png" alt="">
                  <p>PHP4.X</p>
                </div>
              </div>
            </div>
          </div>
        </div>
      </div>
    </section>
    <section class="ourservces-experience">
      <div class="container">
        <div class="ourservces-expertitle">
          <div class="" data-aos="fade-up">
            <h2><span>Experience </span>& Expertise</h2>
            <p>We stand out for 90% client satisfaction</p>
          </div>
          <div class="ourservces-expertiles">
            <ul>
              <li data-aos="flip-left">
                <div class="ourservces-img"><img src="images/experience-01.png" alt=""></div>
                <h4>Dedicated PHP Developers</h4>
                <p>No working hours are wasted in lengthy discussions and communicating with the team-process cycle is completed before time.</p>
              </li>
              <li data-aos="flip-right">
                <div class="ourservces-img"><img src="images/experience-02.png" alt=""></div>
                <h4>Quick Turnaround Time</h4>
                <p>No working hours are wasted in lengthy discussions and communicating with the team-process cycle is completed before time.</p>
              </li>
              <li data-aos="flip-left">
                <div class="ourservces-img"><img src="images/experience-03.png" alt=""></div>
                <h4>20% Average Cost Saving</h4>
                <p>No working hours are wasted in lengthy discussions and communicating with the team-process cycle is completed before time.</p>
              </li>
              <li data-aos="fade-right">
                <div class="ourservces-img"><img src="images/experience-04.png" alt=""></div>
                <h4>Core Competence</h4>
                <p>No working hours are wasted in lengthy discussions and communicating with the team-process cycle is completed before time.</p>
              </li>
              <li data-aos="flip-left">
                <div class="ourservces-img"><img src="images/experience-05.png" alt=""></div>
                <h4>Agile Methodology</h4>
                <p>No working hours are wasted in lengthy discussions and communicating with the team-process cycle is completed before time.</p>
              </li>
              <li data-aos="fade-right">
                <div class="ourservces-img"><img src="images/experience-06.png" alt=""></div>
                <h4>Cohesive Environment</h4>
                <p>No working hours are wasted in lengthy discussions and communicating with the team-process cycle is completed before time.</p>
              </li>
              <li data-aos="flip-left">
                <div class="ourservces-img"><img src="images/experience-07.png" alt=""></div>
                <h4>Strong Skillset</h4>
                <p>No working hours are wasted in lengthy discussions and communicating with the team-process cycle is completed before time.</p>
              </li>
              <li data-aos="fade-right">
                <div class="ourservces-img"><img src="images/experience-08.png" alt=""></div>
                <h4>Adaptive to new versions and technologies</h4>
                <p>No working hours are wasted in lengthy discussions and communicating with the team-process cycle is completed before time.</p>
              </li>
            </ul>
          </div>
        </div>
      </div>
    </section>
    <section class="ourservces-result">
      <div class="container">
        <div class="ourservces-title" data-aos="fade-up">
          <h2>We Generate <span>Outstanding Results</span></h2>
          <p>Most enterprises have transformed their working with our solutions and services</p>
        </div>
        <div class="ourservces-tiles">
          <div class="row">
            <div class="col-xs-12 col-sm-12 col-md-4" data-aos="zoom-in">
              <div class="ourservces-tiles-inner">
                <div class="ourservces-tiles-left">
                  <p>Total Website Developed</p>
                  <p>By AppVoxel Team</p>
                </div>
                <div class="ourservces-tiles-rgt sev-grdnt">700+</div>
              </div>
            </div>
            <div class="col-xs-12 col-sm-12 col-md-4" data-aos="zoom-in">
              <div class="ourservces-tiles-inner">
                <div class="ourservces-tiles-left">
                  <p>Our </p>
                  <p>Happy Clients</p>
                </div>
                <div class="ourservces-tiles-rgt twogrdnt">200+</div>
              </div>
            </div>
            <div class="col-xs-12 col-sm-12 col-md-4" data-aos="zoom-in">
              <div class="ourservces-tiles-inner">
                <div class="ourservces-tiles-left">
                  <p>Total Number of</p>
                  <p>Employees</p>
                </div>
                <div class="ourservces-tiles-rgt one-hun">100+</div>
              </div>
            </div>
          </div>
        </div>
      </div>
    </section>
    <section class="porfolio-slider">
      <div class="container">
        <div class="top-title" data-aos="fade-up">
          <h2>Our <span>Porfolio</span></h2>
          <p>Straightforward solutions to complex business challenges.</p>
          <a href="#">VIEW ALL <i class="fa fa-arrow-right"></i></a>
        </div>
        <div class="porfolio-slder">
          <div id="porfolio" class="owl-carousel owl-theme">
            <div class="item">
              <div class="porfolio-slier">
                <img src="images/portfolio-slide01.jpg" class="img-responsive" alt="">
                <div class="porfolio-sliercontent">
                  <div class="item-contnt">
                    <h5>Web Portal</h5>
                    <h3>M2All - Mahindra</h3>
                  </div>
                  <p>adipiscing elit, sed do eiusmod tempor incididunt ut
                  <p>
                  <p>ad minim veniam, quis nostrud exercitation </p>
                  <a href="#" class="readmore">READ MORE <i class="fa fa-long-arrow-right" aria-hidden="true"></i></a>
                </div>
              </div>
            </div>
            <div class="item">
              <div class="porfolio-slier">
                <img src="images/portfolio-slide02.jpg" class="img-responsive" alt="">
                <div class="porfolio-sliercontent">
                  <div class="item-contnt">
                    <h5>Mobile Application</h5>
                    <h3>ShowTray</h3>
                  </div>
                  <p>adipiscing elit, sed do eiusmod tempor incididunt </p>
                  <p>ad minim veniam, quis nostrud exercitation  </p>
                  <a href="#" class="readmore">READ MORE <i class="fa fa-long-arrow-right" aria-hidden="true"></i></a>
                </div>
              </div>
            </div>
            <div class="item">
              <div class="porfolio-slier">
                <img src="images/portfolio-slide01.jpg" class="img-responsive" alt="">
                <div class="porfolio-sliercontent">
                  <div class="item-contnt">
                    <h5>Web Portal</h5>
                    <h3>M2All - Mahindra</h3>
                  </div>
                  <p>adipiscing elit, sed do eiusmod tempor incididunt ut</p>
                  <p>ad minim veniam, quis nostrud exercitation </p>
                  <a href="#" class="readmore">READ MORE <i class="fa fa-long-arrow-right" aria-hidden="true"></i></a>
                </div>
              </div>
            </div>
            <div class="item">
              <div class="porfolio-slier">
                <img src="images/portfolio-slide02.jpg" class="img-responsive" alt="">
                <div class="porfolio-sliercontent">
                  <div class="item-contnt">
                    <h5>Mobile Application</h5>
                    <h3>ShowTray</h3>
                  </div>
                  <p>adipiscing elit, sed do eiusmod tempor incididunt </p>
                  <p>ad minim veniam, quis nostrud exercitation</p>
                  <a href="#" class="readmore">READ MORE <i class="fa fa-long-arrow-right" aria-hidden="true"></i></a>
                </div>
              </div>
            </div>
          </div>
        </div>
      </div>
    </section>
    <section class="working-gether footer-form">
      <div class="container">
        <div class="row">
          <div class="col-xs-12 col-sm-12 col-md-6" data-aos="fade-right">
            <div class="working-gether-left">
              <img src="images/foot-logo.png" alt="">
              <h3>Interested in <span>working together? </span></h3>
              <p>Let’s collaborate to improve the world through design and technology. </p>
              <div class="working-gethermaillink">
                <a href="#" class="mail-lnk">
                  <i class="fa fa-envelope"></i> 
                  <span class="linktxt">
                    <h5>Mail to our Sales Department</h5>
                    <h4>info@appvoxel.com</h4>
                  </span>
                </a>
                <a href="#" class="mail-lnk">
                  <i class="fa fa-skype"></i> 
                  <span class="linktxt">
                    <h5>Skype</h5>
                    <h4>appvoxel</h4>
                  </span>
                </a>
              </div>
            </div>
          </div>
          <div class="col-xs-12 col-sm-12 col-md-6" data-aos="fade-left">
            <div class="working-gether-rgtform">
              <form action="#" autocomplete="off">
                <fieldset>
                  <input id="first" type="text" name="first" required>
                  <label for="first">Name</label>
                  <div class="after"></div>
                </fieldset>
                <fieldset>
                  <input id="last" type="text" name="last" required>
                  <label for="last">Compony Name</label>
                  <div class="after"></div>
                </fieldset>
                <fieldset>
                  <input id="last" type="email" name="Email" required>
                  <label for="Email">Email</label>
                  <div class="after"></div>
                </fieldset>
                <fieldset>
                  <textarea name="msg" id="msgc" required></textarea>
                  <label for="Email">How can we help?</label>
                  <div class="after"></div>
                  <p class="rest">Rest assured, no details go out of this room.</p>
                </fieldset>
                <fieldset>
                  <button class="btn">get In Touch</button>
                </fieldset>
              </form>
            </div>
          </div>
        </div>
      </div>
    </section>
    <footer class="site-footer">
      <?php include('footer.php') ?>
    </footer>
  </body>
</html>
<script>
  $( document ).ready(function() {
        $('#bannerhh').owlCarousel({
            loop:true,
            margin:10,
            dots:false,
            autoplay:true,
            nav:true,
            //animateOut: 'fadeOut',
            responsive:{
                0:{
                    items:1
                },
                600:{
                    items:1
                },
                1000:{
                    items:1
                }
            }
        })
  });
  
    $( document ).ready(function() {
        $('#porfolio').owlCarousel({
            loop:true,
            margin:10,
            autoplay:true,
            dots:false,
            autoWidth:true,
            nav:true,
            responsive:{
                0:{
                    items:1
                },
                600:{
                    items:2
                },
                1000:{
                    items:2
                },
                1500:{
                    items:2
                }
            }
        })
  });
  
  
    $( document ).ready(function() {
        $('#testomonial').owlCarousel({
            loop:true,
            margin:10,
            dots:true,
            nav:true,
            autoplay:true,
            responsive:{
                0:{
                    items:1
                },
                600:{
                    items:1
                },
                1000:{
                    items:1
                }
            }
        })
  });
</script>


<script>
  AOS.init({
    duration: 1200,
    once: true, // whether animation should happen only once - while scrolling down
    anchorPlacement: 'top-bottom', // defines which position of the element regarding to window should trigger the animation
  })
</script>


<script>
  $('.dropdown').hover(function() {
         $(this).find('.dropdown-menu').stop(true, true).delay(200).fadeIn(500);
       }, 
       function() {
      $(this).find('.dropdown-menu').stop(true, true).delay(200).fadeOut(500);
  });
</script>
<script>
  $(document).ready(function() {
  // executes when HTML-Document is loaded and DOM is ready
  
  // breakpoint and up  
  $(window).resize(function(){
  if ($(window).width() >= 980){  
  
      // when you hover a toggle show its dropdown menu
      $(".navbar .dropdown-toggle").hover(function () {
         $(this).parent().toggleClass("show");
         $(this).parent().find(".dropdown-menu").toggleClass("show"); 
       });
  
        // hide the menu when the mouse leaves the dropdown
      $( ".navbar .dropdown-menu" ).mouseleave(function() {
        $(this).removeClass("show");  
      });
  
    // do something here
  } 
  });  
  // document ready  
  });
</script>


<!-- Request A Quote Modal -->
<div class="modal fade" id="requestaquote" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel" aria-hidden="true">
  <div class="modal-dialog" role="document">
    <div class="modal-content">
      <button type="button" class="close" data-dismiss="modal" aria-label="Close">
      <span aria-hidden="true">&times;</span>
      </button>
      <div class="modal-body">
        <div class="working-gether">
          <div class="container">
            <div class="row">
              <div class="col-xs-12 col-sm-12 col-md-6" data-aos="fade-right">
                <div class="working-gether-left">
                  <img src="images/startup-ideanew.png" alt="">
                  <h3>Request a <span>Quote </span></h3>
                  <p>We’re eager to work with you. Please share your project goals and contact information. We respond to 97% of messages within 1-2 business day. Really!</p>
                  <div class="working-gethermaillink">
                    <a href="#" class="mail-lnk">
                      <i class="fa fa-envelope"></i>
                      <span class="linktxt">
                        <h5>Mail to our Sales Department</h5>
                        <h4>info@appvoxel.com</h4>
                      </span>
                    </a>
                    <a href="#" class="mail-lnk">
                      <i class="fa fa-skype"></i>
                      <span class="linktxt">
                        <h5>Skype</h5>
                        <h4>appvoxel</h4>
                      </span>
                    </a>
                  </div>
                </div>
              </div>
              <div class="col-xs-12 col-sm-12 col-md-6" data-aos="fade-left">
                <div class="working-gether-rgtform">
                  <form action="#" autocomplete="off">
                    <div class="full-width">
                      <fieldset>
                        <input id="first" type="text" name="first" required>
                        <label for="first">Name</label>
                        <div class="after"></div>
                      </fieldset>
                      <fieldset>
                        <input id="Country" type="text" name="Country" required>
                        <label for="Country">Country</label>
                        <div class="after"></div>
                      </fieldset>
                    </div>
                    <fieldset>
                      <input id="Email" type="email" name="Email" required>
                      <label for="Email">Email</label>
                      <div class="after"></div>
                    </fieldset>
                    <div class="full-width">
                      <fieldset>
                        <input id="Phone" type="number" name="Phone" required>
                        <label for="Phone">Phone</label>
                        <div class="after"></div>
                      </fieldset>
                      <fieldset>
                        <input id="Skype" type="text" name="Skype" required>
                        <label for="Skype">WhatsApp No./Skype ID</label>
                        <div class="after"></div>
                      </fieldset>
                    </div>
                    <fieldset>
                      <input id="project" type="text" name="project" required>
                      <label for="project">What’s your project all about?</label>
                      <div class="after"></div>
                    </fieldset>
                    <fieldset>
                      <button class="btn">get In Touch</button>
                    </fieldset>
                  </form>
                </div>
              </div>
            </div>
          </div>
        </div>
      </div>
    </div>
  </div>
</div>